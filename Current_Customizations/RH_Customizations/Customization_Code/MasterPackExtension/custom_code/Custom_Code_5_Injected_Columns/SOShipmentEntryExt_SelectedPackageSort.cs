...
namespace CustomWMS
{
        ...
        : PXGraphExtension<WmsShipmentExt, SOShipmentEntry>
    {
        ...
        private const string Version = "2026-06-17-CUSTOMER-BYPASS-REMAINING-QTY-SKIP-STATUS-TEXT-01";
        ...
        public override void Initialize()
        {
            ...
            PXUIFieldAttribute.SetDisplayName<WmsPlan.shipmentSplitLineNbr>(
                Base.Caches<WmsPlan>(),
                "Remaining Qty");

            PXUIFieldAttribute.SetDisplayName<WmsPlan.orderLineNbr>(
                Base.Caches<WmsPlan>(),
                "Skipped Status");
        }
        ...
        protected virtual IEnumerable selectedPackageContentsView()
        {
            ...
            if (ShouldBypassTopRowWorkflow())
            {
                PXTrace.WriteInformation(
                    $"{TracePrefix} Top-row workflow bypassed. Returning all rows with remaining qty and skipped status, without incomplete-only filter or skip sorting.");
                ...
                {
                    item.Row.ShipmentSplitLineNbr = (int?)item.RemainingQty;
                    item.Row.OrderLineNbr = GetSkipStatusText(item.SkipSortOrder);
            ...
            PXTrace.WriteInformation(
                $"{TracePrefix} Live incomplete-only remaining qty and skipped status complete. Returned={sortedRows.Count}");
        ...
        private List<RowCalc> GetCalculatedRows(SOPackageDetailEx package)
        {
            ...
            foreach (WmsPlan row in plannedRows)
            {
                ...
                SelectedPackageContentsExt rowExt =
                    row.GetExtension<SelectedPackageContentsExt>();

                rowExt.UsrCompletedSortOrder = completed ? 1 : 0;
                rowExt.UsrSkipSortOrder = skipped ? 1 : 0;
