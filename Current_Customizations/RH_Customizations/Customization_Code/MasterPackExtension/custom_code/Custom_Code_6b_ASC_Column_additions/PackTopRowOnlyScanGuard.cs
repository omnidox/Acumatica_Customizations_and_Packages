...
namespace CustomWMS
{
            ...
            .ScanExtension<PickPackShip.PackMode.ConfirmState.Logic>
    {
        ...
        public virtual FlowStatus Confirm(Func<FlowStatus> base_Confirm)
        {
            ...
            {
                PXTrace.WriteInformation($"{TracePrefix} Confirm bypassed top-row scan guard by customer.");
                return base_Confirm();
            }
            ...
            Func<IEnumerable<SOShipLineSplit>> base_GetSplitsToPack)
        {
            ...
            if (ShouldBypassTopRowWorkflow(package))
            {
                PXTrace.WriteInformation($"{TracePrefix} GetSplitsToPack bypassed top-row split filtering by customer.");
                return baseSplits;
            }
