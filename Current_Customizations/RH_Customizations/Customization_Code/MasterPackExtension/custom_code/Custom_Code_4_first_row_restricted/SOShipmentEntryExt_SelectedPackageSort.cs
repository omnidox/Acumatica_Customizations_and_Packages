...
namespace CustomWMS
{
        ...
        : PXGraphExtension<WmsShipmentExt, SOShipmentEntry>
    {
        ...
        {
            PXTrace.WriteInformation($"{TracePrefix} VERSION 2026-05-27-HYBRID-LIVE-SORT-INCOMPLETE-ONLY");
                    ...
                    .Select(row =>
                    {
                        ...
                        if (completed)
                        {
                            PXTrace.WriteInformation(
                                $"{TracePrefix} Completed row hidden: RecordID={row.RecordID}, InventoryID={row.InventoryID}, SplitLineNbr={row.ShipmentSplitLineNbr}, Expected={expectedQty}, Actual={actualQty}");
                        }
            ...
            PXTrace.WriteInformation(
                $"{TracePrefix} Live incomplete-only filter complete. Planned={plannedRows.Count}, Actual={actualRows.Count}, Returned={sortedRows.Count}");
